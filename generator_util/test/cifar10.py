...
import re
import torch
from generator_util.dataloading import MultiThreadedAugmenter
import numpy as np
import os
from generator_util.dataloading.data_loader import DataLoaderFromDataset
from generator_util.datasets.cifar import HighPerformanceCIFARLoader, CifarDataset
from generator_util.transforms.spatial_transforms import SpatialTransform
from generator_util.transforms import NumpyToTensor, Compose
...
if __name__ == '__main__':
    ...
    pin_memory = False
    num_epochs = 3
    dataset_dir = 'E:\\shunjieg\\datasets\\cifar10'
    ...
    for _ in range(num_epochs):
        ...
        for _ in mt:
            batches += 1
    stop = time()
    print('generators took %03.4f seconds' % (stop - start))
    ...
    for _ in range(num_epochs):
        ...
        for _ in mt:
            batches += 1
    stop = time()
    print('high performance generators %03.4f seconds' % (stop - start))
